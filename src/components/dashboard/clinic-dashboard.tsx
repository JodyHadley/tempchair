...
const jobStatusConfig: Record<JobStatus, { label: string; variant: "default" | "secondary" | "destructive" | "outline" }> = {
...
const appStatusConfig: Record<ApplicationStatus, { label: string; variant: "default" | "secondary" | "destructive" | "outline" }> = {
...
export function ClinicDashboard({ clinicId }: { clinicId: string }) {
  ...
  const myJobs = getJobsByClinicId(clinicId);
  const myReviews = getReviewsForUser(clinicId);

  const allApplications = myJobs.flatMap((job) =>
    getApplicationsByJobId(job.id).map((app) => ({ ...app, job }))
  );
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8">
        ...
        <p className="mt-1 text-muted-foreground">
          Manage your positions, applications, and team from your dashboard.
        </p>
      </div>
                  ...
                  {pendingApps === 0 ? (
                    ...
                    <div className="space-y-3">
                      {allApplications
                        ...
                        .map((app) => {
                          const worker = getWorkerById(app.workerId);
                          return (
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                    {worker?.specialty} &middot; {app.job.title} &middot; {app.job.dates}
                                  </p>
                                </div>
                              </div>
                      ...
                      <div className="flex-1">
                        ...
                        {apps.length > 0 && (
                          <div className="mt-3 space-y-2">
                            ...
                            {apps.map((app) => {
                              const worker = getWorkerById(app.workerId);
                              const aConfig = appStatusConfig[app.status];
                              return (
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-[10px] text-muted-foreground">
                                        {worker?.specialty} &middot; {worker?.rating} stars
                                      </p>
                                    </div>
                                  </div>
                                  <Badge variant={aConfig.variant} className="text-[10px]">
                                    {aConfig.label}
                                  </Badge>
                                </div>
                              );
            ...
            {allApplications.length === 0 ? (
              <Card>
                <CardContent className="p-8 text-center text-muted-foreground">
                  No applications received yet. Post a position to attract talent!
                </CardContent>
              </Card>
            ) : (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge variant={aConfig.variant} className="text-xs">
                                {aConfig.label}
                              </Badge>
                            </div>
                            <p className="text-xs text-primary font-medium">{worker?.specialty}</p>
                            ...
                            <div className="mt-2 rounded-md bg-muted/50 px-3 py-2">
                              <p className="text-xs text-muted-foreground">
                                <span className="font-medium">For:</span> {app.job.title} &middot; {app.job.dates}
                              </p>
        ...
        <TabsContent value="reviews">
          <div className="space-y-4">
            {myReviews.length === 0 ? (
              <Card>
                <CardContent className="p-8 text-center text-muted-foreground">
                  No reviews yet. Complete bookings to start building your clinic&apos;s reputation!
                </CardContent>
              </Card>
            ) : (
